(function($) {
  ...
  function authenticate_user($form)
  {
    ...
    ws.onmessage = function (e) {
      ...
      if (data.action == "auth") {
        ...
        ws.send(JSON.stringify({token: data.token, password: password}))
      } else if (data.action == "flag") {
        $('#flag')[0].src = "data:image/png;base64," + data.bytes;
        form_success($form);
      } else {
        form_failed($form);
      }
